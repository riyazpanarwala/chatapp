...
export function initSocket(username, avatar = '') {
  if (socket?.connected) {
    const currentAvatar = typeof window !== 'undefined' ? localStorage.getItem(`chat_avatar:${username}`) : avatar;
    socket.emit('set-username', { username, ...(currentAvatar ? { avatar: currentAvatar } : {}) });
    return socket;
  }
...
// How long we wait for the server's delivery ack before treating a send as
// unconfirmed. Kept as a named constant since it's referenced from the
// timeout branch below and from the UI copy in useChat/MessageList.
...
export async function sendMessage(message, isRetry = false) {
  ...
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(async () => {
      if (settled) return;
      settled = true;
      // The ack never arrived within the timeout — this doesn't necessarily
      // mean the message wasn't delivered (it may still be in flight), but
      // from the client's point of view delivery is now unconfirmed. Surface
      // that as a distinct 'failed' status (rather than leaving it stuck on
      // 'pending' forever) so the UI can show something actionable — a
      // retry affordance — instead of just an indefinitely spinning clock.
      try {
        await updateMessageStatus(message.id, 'failed');
      } catch (err) {
        // Don't let a storage failure (quota exceeded, IndexedDB locked,
        // etc.) leave this promise hanging forever — the in-memory UI
        // update and resolve() below still need to happen regardless.
        console.error('[Socket] Failed to persist failed status:', err);
      }
      emitLocal('message-status-update', { messageId: message.id, status: 'failed' });
      resolve({ error: 'The server did not acknowledge the message', status: 'failed' });
    }, SEND_ACK_TIMEOUT_MS);

    socket.emit('send-message', message, async (ack) => {
      // Always clear the timeout and apply whatever the server actually
      // told us, even if we already resolved the promise via the timeout
      // path above — a late ack still corrects a message that was shown as
      // 'failed' back to 'delivered' once it truly lands.
      clearTimeout(timeout);
      if (ack?.status === 'delivered') {
        try {
          await updateMessageStatus(message.id, 'delivered');
        } catch (err) {
          console.error('[Socket] Failed to persist delivered status:', err);
        }
...
// Manually re-send a message that's sitting in a 'failed' (ack-timeout) or
// still-'pending' state — used by the "retry" action on a stuck message
// bubble. Resets local status back to 'pending' first so the UI reflects
// the retry attempt immediately.
export async function retryPendingMessage(message) {
  try {
    await updateMessageStatus(message.id, 'pending');
  } catch (err) {
    console.error('[Socket] Failed to reset status before retry:', err);
  }
  emitLocal('message-status-update', { messageId: message.id, status: 'pending' });
...
function emitWithAck(event, payload) {
  ...
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      if (settled) return;
      settled = true;
      resolve({ error: 'The server did not respond' });
    }, 5000);
...
export function getMessageEditHistory(roomId, messageId) {
  return emitWithAck('message-edit-history', { roomId, messageId });
}
...
export function loadMessageContext(roomId, messageId, limit = 100) {
  return emitWithAck('message-context', { roomId, messageId, limit });
}
