...
import { getMessages, saveMessage, messageExists, deleteMessage as deleteLocalMessage } from '../lib/indexedDB';
...
export function useChat() {
  ...
  const initUsername = useCallback((name) => {
    ...
    socketClient.initSocket(name, savedAvatar);
  ...
  useEffect(() => {
    ...
    const unsubscribers = [
      ...
      socketClient.on('username-set', () => {
        ...
        if (activeUsername) {
          const roomId = currentRoomRef.current?.id || localStorage.getItem(`chat_last_room:${activeUsername}`);
          if (roomId) {
      ...
      socketClient.on('joined-room', async ({ roomId, name, description, role, messages: serverMsgs, users, pinnedMessages: pinned, isDM, hasMore }) => {
        ...
        // Switching rooms (e.g. clicking a different room in the sidebar)
        // calls joinRoom() directly without an explicit leaveRoom() first,
        // so we've now left the previous room's Socket.IO room server-side
        // and can never receive its "user-stop-typing" event to clear the
        // indicator. Reset it here so a leftover "X is typing…" from the
        // room we just left can't linger in the room we just joined.
        setTypingUsers([]);
        if (typingRoomRef.current && typingRoomRef.current !== roomId) {
          // Previously this only cancelled the local timer and cleared the
          // ref — it never told the *old* room's other members that this
          // user stopped typing, so they could be left with a stuck "X is
          // typing…" indicator until this user re-typed there later.
      ...
      socketClient.on('message-edited', ({ messageId, newContent, editedAt }) => {
        ...
        setMessages(prev => prev.map(m => {
          ...
          updated = { ...m, content: newContent, edited: true, editedAt };
          return updated;
        }));
        // Keep the local cache in sync so a stale (pre-edit) copy of this
        // message doesn't linger in IndexedDB and get shown again the next
        // time it's loaded from the local cache before the server responds.
      ...
      socketClient.on('message-deleted', ({ messageId }) => {
        ...
        // Prune the deleted message from local storage instead of letting
        // it sit there indefinitely — this is what `deleteMessage` in
        // lib/indexedDB.js is for; it just wasn't being called anywhere.
        deleteLocalMessage(messageId).catch(() => {});
      }),
  ...
  // Re-send a message that ended up in 'failed' status because its
  // delivery ack never arrived within the timeout window. Takes the full
  // message object (the caller — a message bubble — already has it) rather
  // than just an id, since resending needs the original content/type/etc.
  const retryMessage = useCallback(async (message) => {
    if (!message) return;
    setMessages(prev => prev.map(m => m.id === message.id ? { ...m, status: 'pending' } : m));
    const result = await socketClient.retryPendingMessage(message);
    if (result?.error) setError(result.error);
  }, []);

  // Fetch a message's edit history on demand (see server's
  // `message-edit-history` handler) — only called when the user actually
  // opens the "view edit history" affordance, so we don't pay for it on
  // every message.
  const getMessageEditHistory = useCallback(async (messageId) => {
    if (!currentRoom) return { error: 'No room selected' };
    return socketClient.getMessageEditHistory(currentRoom.id, messageId);
  }, [currentRoom]);
  ...
  const loadMessageContext = useCallback(async (messageId) => {
    if (!currentRoom || !messageId) return false;
    const result = await socketClient.loadMessageContext(currentRoom.id, messageId, 100);
