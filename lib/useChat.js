...
export function useChat() {
  ...
  const handleTyping = useCallback(() => {
    ...
    typingTimer.current = setTimeout(() => {
      socketClient.sendStopTyping(currentRoom.id);
    }, 2000);
  }, [currentRoom]);
  ...
  const editMessage = useCallback((messageId, newContent) => {
    if (!currentRoom) return;
    socketClient.editMessage(currentRoom.id, messageId, newContent);
    setMessages(prev => prev.map(m =>
  ...
  const deleteMessage = useCallback((messageId) => {
    if (!currentRoom) return;
    socketClient.deleteMessage(currentRoom.id, messageId);
    setMessages(prev => prev.map(m =>
      m.id === messageId ? { ...m, deleted: true, content: '', type: 'deleted' } : m
    ));
    setPinnedMessages(prev => prev.filter(p => p.id !== messageId));
